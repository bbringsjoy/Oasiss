<?php
    ...
    class Usuario {
        ...
        public function salvar() {
            ...
            if (empty($_POST["id"])) {
                $sql = "insert into usuario (id, nome, email, senha, ativo) 
                values (NULL, :nome, :email, :senha, :ativo)";
                $consulta = $this->pdo->prepare($sql);
                ...
                $sql = "update usuario set nome = :nome, email = :email, ativo = :ativo where id = :id limit 1";
                $consulta = $this->pdo->prepare($sql);
                $consulta->bindParam(":nome", $_POST["nome"]);
                $consulta->bindParam(":email", $_POST["email"]);
                ...
                $sql = "update usuario set senha = :senha, nome = :nome, email = :email, ativo = :ativo where id = :id limit 1";
                $consulta = $this->pdo->prepare($sql);
                $consulta->bindParam(":nome", $_POST["nome"]);
                $consulta->bindParam(":email", $_POST["email"]);
                $consulta->bindParam(":senha", $_POST["senha"]);
